    ...
    namespace BogusEdgeRemover
    {
        ...
        public class BogusEdgeRemover : IDrawingPresentationPlugin
        {
            ...
            private bool LinePrimitiveIsNotExternal(
                LinePrimitive linePrimitive,
                List<CachedLine> cachedLines)
            {
                ...
                foreach (var cl in cachedLines)
                {
                    ...
                    if (AreSameLinePrimitives(current, linePrimitive))
                        continue;

                    LineOrArcPrimitiveIsNotExternal(
                        verticalLine01, horizontalLine01,
                        verticalLine05, horizontalLine05,
                        verticalLine09, horizontalLine09,
                        centerPoint,
                        current,
                        null,
                ...
                ModelEdgePair modelEdgeToBeDeleted)
            {
                ...
                double overlapMin = Math.Max(lpMin, tEdge0);
            ...
            private static Point SegmentIntersectionSnap(Point p1, Point p2, Point p3, Point p4)
            {
                ...
                return Distance.PointToPoint(a, b) <= DrawingEpsilon ?
                    new Point((a.X + b.X) / 2.0, (a.Y + b.Y) / 2.0, 0.0) : null;
            }
                ...
                List<CachedLine> cachedLines)
            {
                ...
                {
                    var sp = new Vector2(s.Point1.X, s.Point1.Y);
                    var ep = new Vector2(s.Point2.X, s.Point2.Y);
                    result.Add(new LinePrimitive(sp, ep));
                }
